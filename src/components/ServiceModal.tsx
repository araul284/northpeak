...
export const ServiceModal: React.FC<ServiceModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl bg-white border-2 border-black p-6 sm:p-8 space-y-6 text-left max-h-[90vh] overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 bg-black text-white hover:bg-zinc-800 transition-colors"
        ...
        <div className="space-y-2 pr-8">
          <div className="inline-block px-2.5 py-0.5 bg-black text-white text-[10px] font-bold uppercase tracking-widest font-mono">
        ...
        <div className="space-y-3 pt-2">
          <h3 className="text-xs font-black uppercase tracking-widest text-zinc-500 font-mono">
            Included Deliverables & Scope
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {service.deliverables.map((item, idx) => (
              <div key={idx} className="flex items-start gap-2.5 p-3 border border-black bg-zinc-50 text-black text-xs font-medium">
        ...
        <div className="pt-4 border-t border-black flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-black text-white font-bold text-xs uppercase tracking-widest hover:bg-zinc-800 transition-colors"
          >
